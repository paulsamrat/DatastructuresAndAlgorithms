...
*/
	...
	public static int lastStoneWeight() {
        final int[] stones = new int[] {2,7,4,1,8,1};
        final PriorityQueue<Integer> pq = new PriorityQueue<Integer>(Collections.reverseOrder());
        for (int weight : stones)
        		pq.offer(weight);
        while(pq.size()>1) {
        	final int stone1 = pq.poll();
        	final int stone2 = pq.poll();
        	if (stone1 != stone2)
        		pq.offer(Math.abs(stone2-stone1));
        }
        return pq.isEmpty() ? 0 : pq.poll();
        
    }
	public static int[][] kClosest() {
		int[][] points = {{1,3},{-2,2}};
		int k = 1;
		int [][] res = new int[1][1];
		final PriorityQueue<Double> pq = new PriorityQueue<Double>();
		//compute the euclian distance
		for (int[] point : points)
		{
			int x = point[0];
			int y = point[1];
			
			final double eucliandist  = Math.sqrt(Math.pow(x-0, 2)+Math.pow(y-0, 2));
			pq.offer(eucliandist);
		}
