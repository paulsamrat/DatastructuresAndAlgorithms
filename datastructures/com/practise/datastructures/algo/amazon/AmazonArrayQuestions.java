...
//	/https://www.techiedelight.com/trapping-rain-water-within-given-set-bars/
	...
	private static void trapping_rain_water(final int[] arr)
	{
		...
		// the max'm water that can be collected on top of any bar is the lowest among two left and right highest bars
		System.out.println();
		...
		for (int i = 0 ; i < arr.length; i++)
		{
			// for any bar - the max water that can be stored at that bar will be min of max bar heights on both sides
	...
	{	
		...
		while ( start <  end )
		{
			...
			{
				if (arr[end] >= rightMax)  rightMax = arr[end];
				else totalWater+= rightMax-arr[end];
				--end;
			}
			else
			{
				if (arr[start] >= leftMax) leftMax = arr[start];
				else totalWater+= leftMax-arr[start];
				++start;
			}
		}
